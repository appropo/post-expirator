...
//				return NULL;
			...
			add_action( 'admin_head', array( &$this, 'js_admin_header'));
			...
			load_plugin_textdomain( 'post-expirator', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/');
		...
		function meta_custom() {
			...
			foreach ($custom_post_types as $t) {
				...
				if (!isset($defaults['activeMetaBox']) || $defaults['activeMetaBox'] == 'active') {
					add_meta_box('expirationdatediv', __( 'Post Expirator', 'post-expirator'), array( &$this, 'meta_box'), $t, 'side', 'core');
				}
			}
		}
